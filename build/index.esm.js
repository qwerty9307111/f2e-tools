...
var regExp = {
  ...
  // 非正整数
  REGEXP_NON_NEGATIVE_NUMBER: /^\+?\d*(\.\d+)?$/,
  // 非负数
  REGEXP_NON_POSITIVE_NUMBER: /^((-\d+(\.\d+)?)|(0+(\.0+)?))$/,
  // 非正数
  REGEXP_NUMBER: /^[-\+]?\d*(\.\d+)?$/,
  // 实数
  REGEXP_POSITIVE_NUMBER: /^\+?(([0-9]+\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\.[0-9]+)|([0-9]*[1-9][0-9]*))$/,
  // 正数
  REGEXP_NEGATIVE_NUMBER: /^(-(([0-9]+\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\.[0-9]+)|([0-9]*[1-9][0-9]*)))$/,
  // 负数
  REGEXP_FLOAT: /^[-\+]?([1-9]\d*\.\d+|0\.\d*[1-9]\d*|0?\.0+)$/,
  // 浮点数
  REGEXP_EN_NUM: /^[a-z0-9]+$/i,
  // 英文 + 数字
  REGEXP_PASS_COMPLEX: /(?=(.*[0-9]))(?=.*[\!@#$%^&*()\\[\]{}\-_+=~`|:;"'<>,./?])(?=.*[a-z])(?=(.*[A-Z]))(?=(.*)).{4,}/,
  // 复杂密码（包含大小写字母，数字，特殊字符，至少4个字符长）
  REGEXP_PASS_MODERATE: /(?=(.*[0-9]))((?=.*[A-Za-z0-9])(?=.*[A-Z])(?=.*[a-z]))^.{3,}$/,
  // 中等密码（包含大小写字母，数字，至少3个字符长）
  REGEXP_URL: /^https?:\/\/(([a-zA-Z0-9_-])+(\.)?)*(:\d+)?(\/((\.)?(\?)?=?&?[a-zA-Z#0-9_-](\?)?)*)*$/i,
  // URL 地址
  ...
  REGEXP_TIME_12: /^((1[0-2]|0?[1-9]):([0-5][0-9])(:(?:[012345]\d))? ?([ap]m)?)$/i,
  // HH:mm 12小时制
  REGEXP_TIME_24: /^(?:[01]\d|2[0123]):(?:[012345]\d)(:(?:[012345]\d))?$/,
  // HH:mm 24小时制
  REGEXP_AMOUNT_OF_MONEY: /^([¤฿₵¢₡₫€ƒ₲₭£₤₥₦₱PQR$〒₮₩¥￥₴₪៛﷼₣]|B[rs]|Ft|Rs\.|kr|Lm|Sk|R[p$M]|৲৳|S\/\.|NT\$|zł|руб)?([0-9]+|[0-9]{1,3}(,[0-9]{3})*)(\.[0-9]{1,})?$/,
  // 匹配金额（包含千分符）
  REGEXP_HTML_TAG: /^<([a-z]+)([^<]+)*(?:>(.*)<\/\1>|\s+\/>)$/,
  // 匹配 HTML 标签
  REGEXP_REPEAT_STR: /(\b\w+\b)(?=.*\b\1\b)/,
  // 匹配重复字符串 - 取 HTML 标签名
  REGEXP_LNG_LAT: /^((1[0-7]\d)|\d{1,2})(\.\d{1,})?,(([1-8]\d)|\d)(\.\d{1,})?$/,
  ...
  // 邮编
  REGEXP_FILE_PATH: /^((\/|\.\/|\.\.\/|\\|\/\/|https?:\\\\|https?:\/\/)[a-z0-9 _@\-^!#$%&+={}.\/\\\[\]]+)+\.[a-z]+$/i,
  // 带有后缀名的文件路径
  REGEXP_HEX: /^#?([a-f0-9]{8}|[a-f0-9]{6}|[a-f0-9]{3})$/i,
  // 匹配 HEX 颜色值
  REGEXP_SLUG: /^[a-z0-9-]+$/,
  // 匹配 slug 字符串
  REGEXP_PLATES: /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}[A-Z0-9]{4}[A-Z0-9挂学警港澳]{1}$/i,
  // 车牌号
