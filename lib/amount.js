...
const number2Amount = (num, temp = '0,0', nullFormat = 'N/A') => {
  if (num === null || num === undefined || num === '' || checkType.isNaN(Number(num))) { return nullFormat }
  ...
  let [int, float = ''] = Number(num).toString().split('.')
  let floatNum = math.round(float / Math.pow(10, float.length), floatLength)
  if (floatNum === 1) {
    floatNum = '0.0'.padEnd(floatLength + 2, 0)
    int = Number(int) + 1
  }
  const separator = temp.match(/0(.*?)0/)[1]
  const intString = `${int < 0 ? 0 - int : int}`.replace(/(\d)(?=(?:\d{3})+$)/g, '$1' + separator) // 将整数部分逢三一断
  const floatString = floatLength && `${floatNum}`.split('.')[1] ? '.' + `${floatNum}`.split('.')[1].padEnd(floatLength, 0) : ''
...
const number2Percentage = number => {
  const res = math.multiplication(number, 100)
  if (checkType.isNaN(res)) return null
  return res + '%'
}

const percentage2Number = percentage => math.division(Number.parseFloat(percentage), 100) || null

const int2Chinese = (num, option = { case: 'upper' }) => {
  let number = parseFloat(num)
  if (!checkType.isFinite(number)) { return num }
...
const float2Chinese = (num, option = { case: 'upper', type: 'number' }) => {
  let number = parseFloat(num)
  if (!checkType.isFloat(number)) { return '' }
...
const number2ChineseWithOption = util.curry((option, amount) => {
  ...
  if (checkType.isNaN(Number(amount))) return null
