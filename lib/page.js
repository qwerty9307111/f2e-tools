...
const getScrollTop = dom => {
  if (isHTML(dom)) return dom.scrollTop
  let bodyScrollTop = 0
...
const getScrollHeight = dom => {
  if (isHTML(dom)) return dom.scrollHeight
...
const print = option => {
  ...
  if (!checkType.isObject(options)) { throw new TypeError('option 类型错误') }
  const defaultOption = {
    width: 1000,
    height: 900,
    printClass: 'print',
  ...
  const opts = util.assign(defaultOption, options)
  // 打开一个新窗口
...
const preventScroll = (dom) => {
  ...
  getElement(dom).style['overflow-y'] = 'hidden'
  // getElement(dom).style.position = 'fixed'
...
const recoverScroll = (dom) => {
  if (getElement(dom).dataset.preventScroll === 'true') {
    getElement(dom).dataset.preventScroll = false
    getElement(dom).style['overflow-y'] = 'auto'
    // getElement(dom).style.position = 'static'
