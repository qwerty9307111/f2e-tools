...
const innerProxy = (keyName, type) => new Proxy({}, {
  get (target, key, receiver) {
    const endIndex = Number(key) || ''
    return new RegExp(`${passRegExp[type]}{${Number(keyName)},${endIndex === Infinity || endIndex === -Infinity ? '' : endIndex}}$`)
  }
})

const passProxy = type => new Proxy({}, {
  get (target, key, receiver) {
